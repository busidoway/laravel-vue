...
export default {
    ...
    data: () => ({
        navTitle: "Создание формы обучения",
        dataNav: [
            {
    ...
    created() {
        if (this.$route.params.id) {
            this.navTitle = "Редактирование формы обучения";
            this.fetchData();
        }
    },
    methods: {
        ...
        addNewData() {
            ...
            axios.post('/api/form_education_store', { name: this.name })
                .then(response => {
                    this.response.status = 'success';
                    this.response.info = 'Данные успешно сохранены!';

                    // Перенаправление на страницу редактирования
                    this.$router.push({name: 'FormEducationEdit', params: {id: response.data.id}});
                })
                .catch(error => {
        ...
        updateData() {
            ...
            axios.put(`/api/form_education_update/${id}`, { name: this.name })
                .then(response => {
                    this.response.status = 'success';
        ...
        fetchData() {
            const id = this.$route.params.id;
            axios.get(`/api/form_education_edit/${id}`)
                .then(response => {
        ...
        returnBack() {
            this.$router.push({ name: 'FormEducation' });
        }
    }
}
</script>
