                    ...
                    <td>
                        <div class="btn-group">
                            ...
                            <div class="dropdown-menu py-0">
                                <router-link class="dropdown-item" :to="{ name: 'ExamsEdit', params: { id: item.id } }"><span class="fas fa-edit me-2"></span>Редактирование</router-link>
                                <button type="button" class="dropdown-item text-danger rounded-bottom" data-bs-toggle="modal" data-bs-target="#modal-delete" @click.prevent="getDataItem(item.id, item.name, item.date)">
...
export default {
    ...
    methods: {
        getDataList() {
            axios.get('/api/exams')
                .then(response => {
                    ...
                    this.dataArr = response.data.exams;
                })
                .catch(error => {
        ...
        getDataItem(id, name, date) {
            this.dataItem.id = id;
            this.dataItem.title = name + ' ' + date;
        },
        deleteDataItem(id) {
            if(id) {
                axios.delete(`/api/exam_delete/${id}`)
                    .then(response => {
