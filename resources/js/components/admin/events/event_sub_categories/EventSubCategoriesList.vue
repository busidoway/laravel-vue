<template>
    <div class="event-sub-categories__list">
                    ...
                    <td>
                        <div class="btn-group">
                            ...
                            <div class="dropdown-menu py-0">
                                <router-link class="dropdown-item" :to="{ name: 'EventSubCategoriesEdit', params: { id: item.id } }"><span class="fas fa-edit me-2"></span>Редактирование</router-link>
                                <button type="button" class="dropdown-item text-danger rounded-bottom" data-bs-toggle="modal" data-bs-target="#modal-delete" @click.prevent="getDataItem(item.id)">
...
export default {
    ...
    data: () => ({
        ...
        dataTotal: 0,
        dataCount: 10,
        pagePath: '/admin/event_sub_categories/page/',
        pageCurr: 0,
        loading: true,
        status: {
            success: false,
    ...
    methods: {
        getDataList() {
            axios.get('/api/event_sub_categories')
                .then(response => {
        ...
        deleteDataItem(id) {
            if(id) {
                axios.delete(`/api/event_sub_categories_delete/${id}`)
                    .then(response => {
